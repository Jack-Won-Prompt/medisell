...
<div class="container" style="padding:26px 20px;max-width:860px">
    @forelse($faqs as $category => $items)
        <h3 style="font-size:17px;font-weight:700;color:var(--navy-800);margin:24px 0 12px">{{ $category }}</h3>
        @foreach($items as $faq)
            <details class="form-card" style="margin-bottom:10px;padding:0">
                <summary style="padding:16px 20px;cursor:pointer;font-weight:600;list-style:none;display:flex;align-items:center;gap:10px">
                    <span style="color:var(--red);font-weight:800">Q</span> {{ $faq->question }}
                </summary>
