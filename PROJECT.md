...
- 실연동 엔진 2종(자동선택: SERP 우선 → 파트너스):
  - **SERP(구글 쇼핑) API** — `COUPANG_SERP_API_KEY`(SerpAPI 등). 구글 색인 경유로 쿠팡·타 마켓 경쟁가 조회(차단 우회 대행). 유료·안정.
  - **쿠팡 파트너스 API** — `COUPANG_PARTNERS_*`. 무료지만 파트너스 가입 필요.
- **⚠️ 시뮬레이트 기본**: `COUPANG_SIMULATE` 기본 **true**. 키 설정 + `false` 로 실연동 전환.
- 실측 확인: 쿠팡 직접 크롤링은 **403 봇차단**, WING 판매자 키는 파트너스 API에서 **401 미등록** → 경쟁가는 SERP/파트너스만 가능.
