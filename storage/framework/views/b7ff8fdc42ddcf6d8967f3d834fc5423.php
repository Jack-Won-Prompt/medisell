...
<div class="adm-card">
    <div class="h">
        <span>제품 선택 / 키워드 검색
            <?php if($simulate): ?><span class="pill pill-w">시뮬레이트</span>
            <?php elseif($engine==='serp'): ?><span class="pill pill-y">실연동 · 구글쇼핑(SERP)</span>
            <?php elseif($engine==='partners'): ?><span class="pill pill-y">실연동 · 쿠팡 파트너스</span>
            <?php else: ?><span class="pill pill-n">실연동 키 미설정</span><?php endif; ?>
        </span>
    </div>
    <div style="padding:20px">
        ...
        <div class="ahint" style="margin-top:8px">
            ...
            <?php elseif($engine==='serp'): ?>
                실연동(구글쇼핑) — 구글 쇼핑 색인에서 쿠팡·타 마켓 판매가를 조회합니다.
                <?php if(!config('coupang.serp.coupang_only')): ?><span class="muted">(쿠팡 외 마켓 포함. 쿠팡만 보려면 <code>COUPANG_SERP_COUPANG_ONLY=true</code>)</span><?php endif; ?>
            <?php elseif($engine==='partners'): ?>
